		...
		const char *service)	/* I - Service name or port number */
{
  ...
  // #ifdef DEBUG
  printf("[http-addrlist.c::httpAddrGetList()] (hostname=\"%s\", family=AF_%s, service=\"%s\")\n",
...
#endif /* AF_LOCAL */
  {
	...
	*/
    ...
    if (!getaddrinfo(hostname, service, &hints, &results))
    {
      ...
      */
      printf("[http-addrlist.c::httpAddrGetList()] used getaddrinfo()\n");
	...
	if (current->ai_family == AF_INET || current->ai_family == AF_INET6)
	{
	  ...
	  */
	  printf("[http-addrlist.c::httpAddrGetList()] getaddrinfo() result %p\n", current);
	...
	printf("[http-addrlist.c::httpAddrGetList()] called getservbyname()\n");
      }
	...
	return (NULL);
      ...
      printf("[http-addrlist.c::httpAddrGetList()] port num = %d\n", portnum);
