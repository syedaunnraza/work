    ...
    int        create_dir)		/* I - 1 = create directory, -1 = create w/o logging, 0 = not */
{
  ...
  */
  ...
  printf("[conf.c::cupsdCheckPermissions] %s perms are OK!\n", filename);
  return (0);
}
...
cupsdReadConfiguration(void)
{
  ...
  /// XXX RAZA
  printf("[conf.c::cupsdReadConfiguration()] starting conf\n");
  ...
  */
  ...
  /// XXX RAZA
  printf("[conf.c::cupsdReadConfiguration()] old_server_root = %s\n", old_serverroot);
  printf("[conf.c::cupsdReadConfiguration()] old_request_root = %s\n", old_requestroot);
  ...
  */
  ...
  /// XXX RAZA
  printf("[conf.c::cupsdReadConfiguration()] NumBrowsers = %d\n", NumBrowsers);
  printf("[conf.c::cupsdReadConfiguration()] Browsers = %p\n", Browsers);
  ...
  /// XXX RAZA
  printf("[conf.c::cupsdReadConfiguration()] NumPolled = %d\n", NumPolled);
  printf("[conf.c::cupsdReadConfiguration()] Polled= %p\n", Polled);
  ...
  /// XXX RAZA
  printf("[conf.c::cupsdReadConfiguration()] NumRelays = %d\n", NumRelays);
  if (NumRelays > 0)
  {
  ...
  printf("[conf.c::cupsdReadConfiguration()] deleted all listeners\n");
  ...
  */
  ...
  printf("[conf.c::cupsdReadConfiguration()] server name = %s\n", ServerName);

  cupsdSetStringf(&ServerAdmin, "root@%s", temp);
  printf("[conf.c::cupsdReadConfiguration()] server admin = %s\n", ServerAdmin);
  
  cupsdSetString(&ServerBin, CUPS_SERVERBIN);
  printf("[conf.c::cupsdReadConfiguration()] server bin = %s\n", ServerBin);

  cupsdSetString(&RequestRoot, CUPS_REQUESTS);
  printf("[conf.c::cupsdReadConfiguration()] request root = %s\n", RequestRoot);

  cupsdSetString(&CacheDir, CUPS_CACHEDIR);
  printf("[conf.c::cupsdReadConfiguration()] cache dir = %s\n", CacheDir);

  cupsdSetString(&DataDir, CUPS_DATADIR);
  printf("[conf.c::cupsdReadConfiguration()] data dir = %s\n", DataDir);

  cupsdSetString(&DocumentRoot, CUPS_DOCROOT);
  printf("[conf.c::cupsdReadConfiguration()] document root = %s\n", DocumentRoot);

  cupsdSetString(&AccessLog, CUPS_LOGDIR "/access_log");
  printf("[conf.c::cupsdReadConfiguration()] access log = %s\n", AccessLog);

  cupsdSetString(&ErrorLog, CUPS_LOGDIR "/error_log");
  printf("[conf.c::cupsdReadConfiguration()] error log = %s\n", ErrorLog);

  cupsdSetString(&PageLog, CUPS_LOGDIR "/page_log");
  printf("[conf.c::cupsdReadConfiguration()] page log = %s\n", PageLog);

  cupsdSetString(&Printcap, CUPS_DEFAULT_PRINTCAP);
  printf("[conf.c::cupsdReadConfiguration()] print cap = %s\n", Printcap);

  cupsdSetString(&PrintcapGUI, "/usr/bin/glpoptions");
  printf("[conf.c::cupsdReadConfiguration()] print cap GUI = %s\n", PrintcapGUI);

  cupsdSetString(&FontPath, CUPS_FONTPATH);
  printf("[conf.c::cupsdReadConfiguration()] font path = %s\n", FontPath);

  cupsdSetString(&RemoteRoot, "remroot");
  printf("[conf.c::cupsdReadConfiguration()] remote root = %s\n", RemoteRoot);

  cupsdSetString(&ServerHeader, "CUPS/1.3");
  printf("[conf.c::cupsdReadConfiguration()] server header = %s\n", ServerHeader);

  cupsdSetString(&StateDir, CUPS_STATEDIR);
  printf("[conf.c::cupsdReadConfiguration()] state dir = %s\n", StateDir);

#ifdef HAVE_GSSAPI
  cupsdSetString(&GSSServiceName, CUPS_DEFAULT_GSSSERVICENAME);
  printf("[conf.c::cupsdReadConfiguration()] gsss service name = %s\n", GSSServiceName);
#endif /* HAVE_GSSAPI */
  ...
  printf("[conf.c::cupsdReadConfiguration()] server root = %s\n", ServerRoot);

  printf("[conf.c::cupsdReadConfiguration()] clearing classification = %s\n", Classification);
...
#  ifdef HAVE_CDSASSL
  ...
  printf("[conf.c::cupsdReadConfiguration()] server cert = %s\n", ServerCertificate);
#  else
  cupsdSetString(&ServerCertificate, "ssl/server.crt");
  cupsdSetString(&ServerKey, "ssl/server.key");

  printf("[conf.c::cupsdReadConfiguration()] server key = %s\n", ServerKey);
  printf("[conf.c::cupsdReadConfiguration()] server cert = %s\n", ServerCertificate);
...
#  endif /* HAVE_CDSASSL */
  ...
  */
  ...
  printf("[conf.c::cupsdReadConfiguration()] User = %d\n", User);
  ...
  */
  ...
  printf("[conf.c::cupsdReadConfiguration()] Group = %d\n", Group);
  ...
  */
  ...
  printf("[conf.c::cupsdReadConfiguration()] done with read_configuration \n");
  ...
  printf("[conf.c::cupsdReadConfiguration()] RunUser = %d\n", RunUser);
  ...
  */
  ...
  if (!strncmp(ServerRoot, ServerCertificate, strlen(ServerRoot)))
  {
    ...
    printf("[conf.c::cupsdReadConfiguration()] chown() and chmod() to Server Cert = %s\n", ServerCertificate);
  }
...
#  if defined(HAVE_LIBSSL) || defined(HAVE_GNUTLS)
  ...
  if (!strncmp(ServerRoot, ServerKey, strlen(ServerRoot)))
  {
    ...
    printf("[conf.c::cupsdReadConfiguration()] chown() and chmod() to Server Key = %s\n", ServerKey);
  }
#  endif /* HAVE_LIBSSL || HAVE_GNUTLS */
#endif /* HAVE_SSL */
  ...
  */
  ...
  if (!TempDir)
  {
    ...
    printf("[conf.c::cupsdReadConfiguration()] temp dir = %s\n", TempDir);
  }
  ...
  */
  ...
  cupsdInitEnv();
  printf("[conf.c::cupsdReadConfiguration()] done with env init\n");
  ...
  */
  ...
  printf("[conf.c::cupsdReadConfiguration()] configured for up to %d clients\n", MaxClients);
  ...
  */
  ...
  if (Classification)
    ...
    printf("[conf.c::cupsdReadConfiguration()] security set to %s\n", Classification);
  ...
  */
  ...
  printf("[conf.c::cupsdReadConfiguration()] allowing up to %d clients per host \n", MaxClients);
  ...
  */
  ...
  printf("[conf.c::cupsdReadConfiguration()] Num Policies = %d\n", NumPolicies);
  ...
  for (i = 0; i < NumPolicies; i ++) 
    ...
    //              Policies[i]->name);
      ...
      printf("[conf.c::cupsdReadConfiguration()] Policies[%d]=\"%s\"\n", i,
	     Policies[i]->name);
    }
  ...
  */
      ...
      !old_requestroot || !RequestRoot || strcmp(old_requestroot, RequestRoot))
  {
    ...
    printf("[conf.c::cupsdReadConfiguration()] Full reload is required\n");
    ...
    */
	...
	printf("[conf.c::cupsdReadConfiguration()] freeing mime type [%d]\n", i);
	}
      free(MimeTypes);
    }
    ...
    */
    ...
    printf("[conf.c::cupsdReadConfiguration()] Loaded MIME database from \'%s\': %d types, %d filters...\n",
	   ServerRoot, mimeNumTypes(MimeDatabase),
	   mimeNumFilters(MimeDatabase));
    ...
    */
    ...
    for (type = mimeFirstType(MimeDatabase);
	 ...
	 type = mimeNextType(MimeDatabase))
      {
	printf("[conf.c::cupsdReadConfiguration()] type %s/%s\n",
    ...
    for (filter = mimeFirstFilter(MimeDatabase);
	 ...
	 filter = mimeNextFilter(MimeDatabase))
      {
	printf("[conf.c::cupsdReadConfiguration()] filter %s/%s to %s/%s %d %s\n",
	    ...
	    int         defport)	/* I - Default port */
{
  ...
  */
  ...
  printf("[conf.c::get_address] getting address from %s\n", value);
  ...
  */
  ...
  printf("[conf.c::get_address] printing addresses for %s\n", value);  
  
  http_addrlist_t *cur = addrlist;
  while(cur != (http_addrlist_t*)NULL)
    {
      http_addr_t addr = cur->addr;
      struct sockaddr a = addr.addr;
      if (a.sa_family == AF_UNIX)
	{
	  struct sockaddr_un un = addr.un;
	  printf("AF_UNIX %s\n", un.sun_path);
	}
      else if (a.sa_family == AF_INET6)
	{
	  struct sockaddr_in6 in = addr.ipv6;
	  char dest[256];
	  printf("AF_INET6 port=%d flow=%d addr =%s, sid=%d\n", ntohs(in.sin6_port), in.sin6_flowinfo, 
		 inet_ntop(AF_INET6, &in.sin6_addr, dest, 256), in.sin6_scope_id);
	}
      else if (a.sa_family == AF_INET)
	{
	  printf("AF_INET\n");
	  struct sockaddr_in in = addr.ipv4;
	  char dest[256];
	  printf("AF_INET port=%d addr =%s, szero=%s\n", ntohs(in.sin_port), 
		 inet_ntop(AF_INET, &in.sin_addr, dest, 256), in.sin_zero);
  ...
  while (cupsFileGetConf(fp, line, sizeof(line), &value, &linenum))
  {
    ...
    */
    ...
    else if (!strcasecmp(line, "FaxRetryInterval") && value)
    {
      ...
      printf("[conf.c::read_configuration] JobRetryInterval = %d\n", JobRetryInterval);
    ...
    else if (!strcasecmp(line, "FaxRetryLimit") && value)
    {
      ...
      printf("[conf.c::read_configuration] JobRetryLimit = %d\n", JobRetryLimit);
	...
	*/
	...
	httpAddrString(&lis->address, temp, sizeof(temp));
	  ...
	  printf("[conf.c::read_configuration] Listening to %s:%d (IPv6)\n", temp,
	...
	//                 ntohs(lis->address.ipv6.sin6_port));
	  ...
	  printf("[conf.c::read_configuration] Listening to %s (Domain)\n", temp);
          // cupsdLogMessage(CUPSD_LOG_INFO, "Listening to %s (Domain)", temp);
	else
#endif /* AF_LOCAL */
	  printf("[conf.c::read_configuration] Listening to %s:%d (IPv4)\n", temp,
	...
	*/
	...
	if (addr)
	{
	  ...
	  printf("[conf.c::read_configuration] sending browsing info to %s:%d (IPv4)\n",
		 temp, ntohs(dira->to.ipv4.sin_port));
	  NumBrowsers ++;
	}
	else
    ...
    else if (!strcasecmp(line, "BrowseOrder") && value)
    {
      ...
      */
      printf("[conf.c::read_configuration] browse order)\n");
             ...
             !strcasecmp(line, "BrowseRemoteProtocols"))
    {
      ...
      */
      printf("[conf.c::read_configuration] browse protocols\n");
              ...
              !strcasecmp(line, "BrowseDeny")) && value)
    {
      ...
      */
      printf("[conf.c::read_configuration] browse allow deny from host/ip\n");
	...
	*/
    ...
    {
      printf("[conf.c::read_configuration] browse relay\n");
	...
	*/
    ...
    else if (!strcasecmp(line, "BrowsePoll") && value)
    {
      ...
      printf("[conf.c::read_configuration] browse poll\n");
	...
	else
	{
      ...
      */
      ...
      //      cupsdLogMessage(CUPSD_LOG_INFO, "Polling %s:%d", 
      //	      pollp->hostname,
      //              pollp->port);
      printf("[conf.c::read_configuration] Polling %s:%d\n",
	     pollp->hostname,
	     pollp->port);
    }
    else if (!strcasecmp(line, "DefaultAuthType") && value)
    {
      ...
      */
      printf("[conf.c::read_configuration] default auth type\n");
    ...
    {
      printf("[conf.c::read_configuration] default encryption\n");
     /*
...
#  endif /* HAVE_GSSKRB5_REGISTER_ACCEPTOR_IDENTITY */
	...
	printf("[conf.c::read_configuration] user id to run as = %d\n", User);
	...
	if (group != NULL)
	  ...
	  cupsdLogMessage(CUPSD_LOG_ERROR,
	                  ...
	                  value, linenum);
	printf("[conf.c::read_configuration] group id  = %d\n", Group);
      }
    }
    else if (!strcasecmp(line, "SystemGroup") && value)
    {
      ...
      */
      printf("[conf.c::read_configuration] system group \n");
    ...
    else if (!strcasecmp(line, "HostNameLookups") && value)
    {
      ...
      * Do hostname lookups?
      */
      printf("[conf.c::read_configuration] hostname lookups \n");
    ...
    else if (!strcasecmp(line, "LogLevel") && value)
    {
      ...
      */
      ...
      printf("[conf.c::read_configuration] log level \n");
    ...
    else if (!strcasecmp(line, "PrintcapFormat") && value)
    {
      ...
      */
      printf("[conf.c::read_configuration] print cap format \n");
    ...
    else if (!strcasecmp(line, "ServerTokens") && value)
    {
      ...
      */
      printf("[conf.c::read_configuration] server tokens \n");
    ...
    else if (!strcasecmp(line, "PassEnv") && value)
    {
      ...
      */
      printf("[conf.c::read_configuration] pass env \n");
    ...
    else if (!strcasecmp(line, "SetEnv") && value)
    {
      ...
      */
      printf("[conf.c::read_configuration] set env \n");
	      ...
	      int         linenum)	/* I - Current line number */
{
  ...
  printf("[conf.c::read_location] decoding %s:\n", location);
	    ...
	    int         linenum)	/* I - Current line number */
{
  ...
  printf("[conf.c::read_policy] decoding %s:\n", policy);
