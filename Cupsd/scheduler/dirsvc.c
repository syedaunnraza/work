...
cupsdLoadRemoteCache(void)
{
  ...
  */
  ...
  if (!Browsing || !(BrowseRemoteProtocols & BROWSE_CUPS))
  {
    ...
    printf("[dirsvc.c::cupsdLoadRemoteCache] Not loading remote cache.\n");
    return;
  }
  ...
  */
  ...
  if ((fp = cupsFileOpen(line, "r")) == NULL)
    {
        printf("[dirsvc.c::cupsdLoadRemoteCache] could not load remote cache from %s\n", line);
      return;
    }

  printf("[dirsvc.c::cupsdLoadRemoteCache] loading remote cache from %s\n", line);
  ...
  */
  ...
  printf("[dirsvc.c::cupsdLoadRemoteCache] done loading remote cache from %s\n", line);
}
...
{
  printf("[dirsvc.c::cupsdRegisterPrinter(%s)] called\n", p->name);
  if (!Browsing || !BrowseLocalProtocols || !BrowseInterval || !NumBrowsers ||
      (p->type & (CUPS_PRINTER_REMOTE | CUPS_PRINTER_IMPLICIT)))
    {
      printf("[dirsvc.c::cupsdRegisterPrinter] (%s) returning immediately\n", p->name);
      return;
    }
#ifdef HAVE_LIBSLP
/*  if (BrowseLocalProtocols & BROWSE_SLP)
...
cupsdStartBrowsing(void)
{
  ...
  BrowseNext = NULL;
  printf("[dirsvc.c::cupsdStartBrowsing()] entered \n");
  
  if (!Browsing || !(BrowseLocalProtocols | BrowseRemoteProtocols)) 
    {
      printf("[dirsvc.c::cupsdStartBrowsing()] returning immediately, browsing off \n");
      return;
    }
  if ((BrowseLocalProtocols | BrowseRemoteProtocols) & BROWSE_CUPS)
  {
    if (BrowseSocket < 0)
    {
      ...
      */
      ...
      {
	printf("[dirsvc.c::cupsdStartBrowsing()] unable to create browse socket\n");
	...
	return;
      ...
      */
      ...
      {
	printf("[dirsvc.c::cupsdStartBrowsing()] unable to bind browse socket\n");
    ...
    */
    ...
    if (setsockopt(BrowseSocket, SOL_SOCKET, SO_BROADCAST, &val, sizeof(val)))
    {
      cupsdLogMessage(CUPSD_LOG_ERROR, "Unable to set broadcast mode - %s.",
        	      strerror(errno));
      printf("[dirsvc.c::cupsdStartBrowsing()] unable to set broadcast mode\n");
    ...
    */
    if (BrowseRemoteProtocols & BROWSE_CUPS)
    {
      ...
      */
      printf("[dirsvc.c::cupsdStartBrowsing()] listening to browse socket for remote printers\n");
  ...
  if ((BrowseLocalProtocols | BrowseRemoteProtocols) & BROWSE_SLP)
  {
    ...
    */
    ...
    if (SLPOpen("en", SLP_FALSE, &BrowseSLPHandle) != SLP_OK)
    {
      ...
      printf("[dirsvc.c::cupsdStartBrowsing()] unable to open an SLP handle, disabling SLP browsing\n");
  ...
  if ((BrowseLocalProtocols | BrowseRemoteProtocols) & BROWSE_LDAP)
  {
	...
	printf("[dirsvc.c::cupsdStartBrowsing()] cupsdStartBrowsing: Setting CA certificate file \"%s\"\n",
	       BrowseLDAPCACertFile);
  ...
  */
  ...
  printf("[dirsvc.c::cupsdStartBrowsing()] Done\n");
}
...
cupsdStartPolling(void)
{
  ...
  */
  printf("[dirsvc.c::cupsdStartPolling()]:: cupsdStartPolling called\n");
  ...
  if (NumPolled == 0 || BrowseSocket < 0)
  {
    ...
    printf("[dirsvc.c::cupsdStartPolling()]:: cupsdStartPolling returning immediately\n");
    return;
  }
  ...
  */
  ...
  printf("[dirsvc.c::cupsdStartPolling()]:: setting up print arguments for polld \n");
  ...
  * polling daemon...
  */
  printf("[dirsvc.c::cupsdStartPolling()]:: open a pipe for status message from each daemon \n");
  ...
  */
  ...
  for (i = 0, pollp = Polled; i < NumPolled; i ++, pollp ++)
  {
    ...
    if (cupsdStartProcess(polld, argv, envp, -1, -1, statusfds[1], -1, -1,
                          0, &(pollp->pid)) < 0)
    {
      printf("[dirsvc.c::cupsdStartPolling()]::Unable to fork polling daemon - %s\n",
	     strerror(errno));
      ...
      printf("[dirsvc.c::cupsdStartPolling()]:: Started polling daemon for %s:%d, pid = %d\n",
  ...
  */
  ...
  printf("[dirsvc.c::cupsdStartPolling()]:: done\n");
}
    ...
    void		*context)	/* I - User-defined context */
{
  ...
  printf("[dirsvc.c::dnssdRegisterCallback()] name %s, %s)\n", name, regtype);

  if (errorCode)
  {
    printf("[dirsvc.c::dnssdRegisterCallback()] name %s, %s FAILED WITH ERROR %d)\n", name, regtype, (int)errorCode);
    cupsdLogMessage(CUPSD_LOG_ERROR, 
...
dnssdRegisterPrinter(cupsd_printer_t *p)/* I - Printer */
{
  ...
  printf("[dirsvc.c::dnssdRegisterPrinter()] name=%s %s\n", p->name,
                  !p->dnssd_ipp_ref ? "new" : "update");
  ...
  */
  ...
  if (!p->dnssd_ipp_ref)
  {
    ...
    */
    ...
    printf("[dirsvc.c::dnssdRegisterPrinter()] name %s, type %s)\n", p->name, regtype);
