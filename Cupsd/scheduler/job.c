...
cupsdCheckJobs(void)
{
       ...
       job = (cupsd_job_t *)cupsArrayNext(ActiveJobs))
  {
    ...
    */
    ...
    printf("[job.c::cupsdCheckJobs()] Job %d: state_value=%d, loaded=%s",
	...
	*/
        ...
        if (pclass)
	{
	  ...
	  * Start the job...
	  */
	  printf("[job.c::cupsdCheckJobs()] startomg job %d on %s\n", job->id, job->dest);
  ...
  printf("[job.c::cupsdCheckJobs()] done\n");
}
...
cupsdFreeAllJobs(void)
{
  ...
  printf("[job.c::cupsdFreeAllJobs() Jobs = %p\n", Jobs);
  ...
  printf("[job.c::cupsdFreeAllJobs() forcing all jobs to stop\n");
  cupsdStopAllJobs(1);
  
  printf("[job.c::cupsdFreeAllJobs() saving all jobs\n");
  cupsdSaveAllJobs();
       ...
       job = (cupsd_job_t *)cupsArrayNext(Jobs))
  {
    ...
    printf("[job.c::cupsdFreeAllJobs() job = %p\n", job);
  }
...
cupsdLoadAllJobs(void)
{
  ...
  */
  ...
  printf("[job.c::cupsdLoadAllJobs()] stat cache file %s\n", filename);

  if (stat(filename, &fileinfo))
  {
    fileinfo.st_mtime = 0;

    printf("[job.c::cupsdLoadAllJobs()] couldn't stat cache = %s\n", filename);
  ...
  printf("[job.c::cupsdLoadAllJobs()] stat request root = %s\n", RequestRoot);
  
  if (stat(RequestRoot, &dirinfo))
  {
    dirinfo.st_mtime = 0;

    printf("[job.c::cupsdLoadAllJobs()] couldn't stat request root = %s\n", RequestRoot);
  ...
  */
  ...
  printf("[job.c::cupsdLoadAllJobs()] comparing dir time %x with cache time %x\n", (int)dirinfo.st_mtime, (int)fileinfo.st_mtime);

  if (dirinfo.st_mtime > fileinfo.st_mtime)
  {
    printf("[job.c::cupsdLoadAllJobs()] request root = %s is more recent!\n", RequestRoot);
    ...
    {
      printf("[job.c::cupsdLoadAllJobs()] cache %s is more recent!\n", filename);
...
cupsdLoadJob(cupsd_job_t *job)		/* I - Job */
{
  ...
  */
  ...
  printf("[job.c::cupsdLoadJob] loading job from %s...\n", jobfile);
  if ((fp = cupsFileOpen(jobfile, "r")) == NULL)
  {
    printf("[job.c::cupsdLoadJob] unable to load job from %s...\n", jobfile);
    cupsdLogMessage(CUPSD_LOG_ERROR,
  ...
  if (!job->num_files)
  {
    ...
    */
    ...
    for (fileid = 1; fileid < 10000; fileid ++)
    {
      ...
      printf("[job.c::cupsdLoadJob] loading job document file %s...\n", jobfile);
  ...
  */
  ...
  if (job->state_value < IPP_JOB_STOPPED)
  {
    ...
    printf("[job.c::cupsdLoadJob] loading job auth file %s...\n", jobfile);
  ...
  job->access_time = time(NULL);
  printf("[job.c::cupsdLoadJob] done loading job, setting access time = %x\n", (int)job->access_time);
}
...
cupsdSaveAllJobs(void)
{
  ...
  printf("[job.c::cupsdSaveAllJobs()] Saving job cache file %s\n", temp);
  ...
  */
  ...
  printf("[job.c::cupsdSaveAllJobs()] Writing HEADER:\n");
  printf("[job.c::cupsdSaveAllJobs()] <1>: Job cache file for " CUPS_SVERSION "\n");
  printf("[job.c::cupsdSaveAllJobs()] <2>: Written by cupsd on %s\n", temp);
  printf("[job.c::cupsdSaveAllJobs()] <2>: NextJobId %d\n", NextJobId);
  printf("[job.c::cupsdSaveAllJobs()] Done with HEADER:\n");
  ...
  */
       ...
       job = (cupsd_job_t *)cupsArrayNext(Jobs))
  {
    ...
    printf("[job.c::cupsdSaveAllJobs()] Writing Job %d\n", job->id);
  }
...
load_job_cache(const char *filename)	/* I - job.cache filename */
{
  ...
  */
  printf("[job.c::load_job_cache()] called %s\n", filename);
  ...
  */
  ...
  printf("[job.c::load_job_cache()] loading job cache %s\n", filename);
...
load_next_job_id(const char *filename)	/* I - job.cache filename */
{
  ...
  */
  ...
  printf("[job.c::load_next_job_id] Loading NextJobId from job cache file \"%s\"...\n", filename);
  ...
  printf("[job.c::load_next_job_id] NextJobId =%d\n", NextJobId);
  cupsFileClose(fp);
}
...
load_request_root(void)
{
  ...
  */
  ...
  printf("[job.c::load_request_root] Scanning %s for jobs...\n", RequestRoot);
  ...
  if ((dir = cupsDirOpen(RequestRoot)) == NULL)
  {
    cupsdLogMessage(CUPSD_LOG_ERROR,
                    ...
                    RequestRoot, strerror(errno));
    printf("[job.c::load_request_root] unable to open %s for jobs (%s)...\n", RequestRoot,
	   strerror(errno));
    return;
  }
  ...
  */
  ...
  while ((dent = cupsDirRead(dir)) != NULL)
	...
	return;
      ...
      * Assign the job ID...
      */
      printf("[job.c::load_request_root] job from %s\n", dent->filename);
  ...
  cupsDirClose(dir);
  printf("[job.c::load_request_root] Done scanning %s for jobs...\n", RequestRoot);
  printf("[job.c::load_request_root Done, next job id = %d\n", NextJobId);
}
