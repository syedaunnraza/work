...
cupsdDeleteAllListeners(void)
{
  ...
  printf("[listen.c::cupsdDeleteAllListeners()] deleting, Listeners = %p\n", Listeners);
  for (lis = (cupsd_listener_t *)cupsArrayFirst(Listeners);
       lis;
       lis = (cupsd_listener_t *)cupsArrayNext(Listeners))
    {
      printf("[listen.c::cupsdDeleteAllListeners()] free lis = %p\n", lis);
      free(lis);
    }
...
cupsdResumeListening(void)
{
  ...
  printf("[listen.c::cupsdResumeListening()]: num listeners = %d\n",
  ...
  printf("[listen.c::cupsdResumeListening()]: done\n");
...
cupsdStartListening(void)
{
  ...
  printf("[listen.c::cupsdStartListening()]: entered, %d Listeners\n", cupsArrayCount(Listeners));
  ...
  */
  ...
  if ((ServerAddrs = httpAddrGetList(ServerName, AF_UNSPEC, NULL)) == NULL) {
    cupsdLogMessage(CUPSD_LOG_ERROR,
		    ...
		    ServerName);
    printf("[listen.c::cupsdStartListening()]: unable to find ip address for server name %s\n", ServerName);
  }
    ...
    */
    ...
    if (lis->fd == -1)
    {
      ...
      */
      ...
      if (lis->fd == -1)
      {
	printf("[listen.c::cupsdStartListening()]: Unable to open listen socket for address %s:%d - %s.\n",
	       s, p, strerror(errno));
...
#  endif /* IPV6_V6ONLY */
	...
	*/
	...
	umask(mask);
      ...
      if (status < 0)
      {
	printf("[listen.c::cupsdStartListening()]: Unable to bind socket for address %s:%d - %s.\n",
	       s, p, strerror(errno));
	...
	printf("[listen.c::cupsdStartListening()]: Unable to listen for clients on address %s:%d - %s.\n",
	       s, p, strerror(errno));
	...
	printf("[listen.c::cupsdStartListening()]: listening to %s:%d on fd %d.\n",
	       s, p, lis->fd);		
      }
    else
      {
	...
	printf("[listen.c::cupsdStartListening()]: listening to %s on fd %d.\n",
	       s, lis->fd);		
	...
	printf("[listen.c::cupsdStartListening()]: unable to change perms on domain socket %s -- %s\n",
	       s, strerror(errno));
  ...
  */
  ...
  printf("[listen.c::cupsdStartListening()]: LocalPort = %d\n",
	 LocalPort);
 /*
