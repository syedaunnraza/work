...
cupsdCreateCommonData(void)
{
  ...
  */
  ...
  if ((dir = cupsDirOpen(filename)) != NULL)
  {
    ...
    printf("[printers.c: createCommonData] reading %s\n", filename);
...
cupsdDeleteAllPrinters(void)
{
  ...
  printf("[printers.c::cupsdDeleteAllPrinters()] called\n");
  ...
  for (p = (cupsd_printer_t *)cupsArrayFirst(Printers);
    ...
    if (!(p->type & CUPS_PRINTER_CLASS))
      {
	printf("[printers.c::cupsdDeletePrinter()] p = %p\n", p);
...
cupsdLoadAllPrinters(void)
{
  ...
  */
  ...
  printf("[printers.c::cupsdLoadAllPrinters] opening printer conf file %s\n", line);
  ...
  */
  ...
  while (cupsFileGetConf(fp, line, sizeof(line), &value, &linenum))
  {
	...
	*/
	...
	printf("[printers.c::cupsdLoadAllPrinters] loading printer %s\n", value);
	...
	*/
	    ...
	    p->state != IPP_PRINTER_STOPPED)
	{
	  ...
	  */
	  ...
	  printf("[printers.c::cupsdLoadAllPrinter] backend =  %s\n", line);
          ...
          if (access(line, 0))
	  {
	    ...
	    */
	    ...
	    {	  
	      printf("[printers.c::cupsdLoadAllPrinter] backend =  %s exists\n", line);
	    }
        }
  ...
  */
  ...
  else
  {
	...
	*/
	...
	printf("[printers.c::cupsdLoadAllPrinter] reading printer .ppd file %s\n", filename);
	...
	*/
	...
	if (!access(filename, X_OK))
	{
	  ...
	  */
	  ...
	  printf("[printers.c::cupsdLoadAllPrinter] reading printer interface file %s\n", filename);
...
cupsdWritePrintcap(void)
{
  ...
  */
  ...
  printf("[printers.c::cupsdWritePrintcap] opening %s\n", Printcap);
  
  if ((fp = cupsFileOpen(Printcap, "w")) == NULL) 
    {
      printf("[printers.c::cupsdWritePrintcap] couldn't open %s, returning\n", Printcap);
      return;
    }
 /*
  ...
  * Close the file...
  */
  printf("[printers.c::cupsdWritePrintcap] done \n");
  cupsFileClose(fp);
}
