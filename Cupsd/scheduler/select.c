...
#  ifdef HAVE_EPOLL
  if (cupsd_epoll_fd >= 0)
  {
    ...
    printf("[select.c::cupsdAddSelect()]: calling epoll_ctl(...) added = %d\n", added);
    event.events = 0;
...
#  endif /* HAVE_EPOLL */
  ...
  */
  ...
  printf("[select.c::cupsdAddSelect()]: done\n");
  return (1);
}
...
cupsdDoSelect(long timeout)		/* I - Timeout in seconds */
{
  ...
  printf("[select.c::cupsdDoSelect()] polling %d fds for %ld seconds\n",
	 cupsArrayCount(cupsd_fds), timeout);
...
cupsdStartSelect(void)
{
...
#if UNDERSTAND_CUPS
  printf("[select.c::cupsdStartSelect()] just entered fxn, allocated an array of cupsd_fds\n");
#endif

#if defined(HAVE_EPOLL) || defined(HAVE_KQUEUE)
  cupsd_inactive_fds = cupsArrayNew((cups_array_func_t)compare_fds, NULL);
  printf("[select.c::cupsdStartSelect()] system is using epoll/kqueue\n");
#endif /* HAVE_EPOLL || HAVE_KQUEUE */
  ...
  cupsd_update_pollfds = 0;
  printf("[select.c::cupsdStartSelect()] system is using epoll\n");
#elif defined(HAVE_KQUEUE)
  cupsd_kqueue_fd      = kqueue();
  cupsd_kqueue_changes = 0;
  cupsd_kqueue_events  = calloc(MaxFDs, sizeof(struct kevent));
  printf("[select.c::cupsdStartSelect()] system is using kqueue\n");
#elif defined(HAVE_POLL)
  cupsd_update_pollfds = 0;
  printf("[select.c::cupsdStartSelect()] system is using simple poll \n");
#else /* select() */
