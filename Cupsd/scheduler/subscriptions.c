...
cupsdDeleteAllSubscriptions(void)
{
  ...
  printf("[subscriptions.c::cupsdDeleteAllSubscriptions()] Subscriptions = %p\n", Subscriptions);
  if (!Subscriptions)
    return;
  ...
  for (sub = (cupsd_subscription_t *)cupsArrayFirst(Subscriptions);
       ...
       sub = (cupsd_subscription_t *)cupsArrayNext(Subscriptions)) 
    {
      printf("[subscriptions.c::cupsdDeleteAllSubscriptions()] sub = %p\n", sub);
...
cupsdLoadAllSubscriptions(void)
{
  ...
  */
  ...
  printf("[subscriptions.c::cupsdLoadAllSubscriptions] loading subscriptions file %s...\n", line);
  ...
  if ((fp = cupsFileOpen(line, "r")) == NULL)
  {
    ...
    printf("[job.c::cupsdLoadJob] unable to load subscriptions file %s...\n", line);
    return;
  }
