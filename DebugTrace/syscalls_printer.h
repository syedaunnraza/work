...
const int JUST_CREATED = 11; /* after socket() */
const int BOUND_SOCKET = 12; /* after bind() or connect()*/
const int EPHEMERAL_SOCKET=13; /* if listen() or bind() from state = 0 */
const int CLOSED_SOCKET=14; /* keep closed sockets around (they usually get reused) */
const int ACCEPTED_SOCKET=15; /* peer connection from accept */

LOCALVAR map<int, int> socket_states;
LOCALVAR map<int, int> socket_translation_table;

//pr_output stream
LOCALVAR std::ofstream pr_out;
		      ...
		      SYSCALL_STANDARD std, VOID *v)
{
  ...
  switch( call_number )
	...
	socket_states[sockfd]=BOUND_SOCKET;
      }
      break;
    case 3:
      // connect()
	...
	print_sockaddr(my_addr);
	if (socket_states[sockfd] == JUST_CREATED)
	  {
	    pr_out << "\t <--- THIS SOCKET (" << sockfd << ") IS A CANDIDATE FOR EPHEMERAL SOCKET --->" << endl; 
	    socket_states[sockfd] = EPHEMERAL_SOCKET;
	  }
	
	socket_states[PIN_GetSyscallReturn(ctxt, std)] = ACCEPTED_SOCKET;
      }
      break;
    case 6:
      // getsockname()
	...
	if (socket_states[sockfd] == EPHEMERAL_SOCKET)
	  {
	    sa_family_t fam = my_addr->sa_family;
	    if (fam == AF_INET)
	      {
		 struct sockaddr_in *in = (struct sockaddr_in*) my_addr;
		 pr_out << "\t" << "<----port = " << ntohs(in->sin_port) << " is sketchy!" << endl;
	      }
	    else if (fam == AF_INET6)
	      {
		struct sockaddr_in6 *in = (struct sockaddr_in6*) my_addr;
		pr_out << "\t" << "<----port = " << ntohs(in->sin6_port) << " is sketchy!" << endl;
	      }
	  }
      }
      break;
		  ...
		  SYSCALL_STANDARD std, VOID *v)
{
  ...
  if (only_sockets && (pr_last_syscall_number == 102 || pr_last_syscall_number == 6))
    { 
  ...
  switch( pr_last_syscall_number )
	...
	socket_states[fd] = CLOSED_SOCKET;
      }
      break;
    case 7:
      // waitpid()
