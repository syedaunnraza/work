...
def cfg_visualize_files(file1, file2):
    handle1 = open(file1)
    handle2 = open(file2)
    ...
    while(line1 != '' and line2 != ''):
        if (line1[0] == 'r' and line1.find('0x') == -1):
            line1 = handle1.readline()
            continue
        if (line2[0] == 'r' and line2.find('0x') == -1):
            line2 = handle2.readline()
            continue
        if line1 != line2:
            new_interval = (num_ins, num_ins+1, False)
...
def main(args):
    if len(args) != 3:
        ...
        print 'starting control flow analysis'
        intervals_cfg = cfg_visualize_files(args[0], args[1])
        print intervals_cfg
        draw_trace(intervals_cfg, args[2], -1, True)
        return
