...
Ideally, any organization would assign dedicated
machines for individual server applications
to effectively isolate them. 
However, this approach is inefficient
because individual applications tend to utilize only a modest fraction of 
...
allow a host to run VMs with
combined memory requirements
that exceed the total underlying physical memory
available \cite{waldspurger2002memory}. This scenario 
is called {\em memory overcommit}.
...
\end{itemize}
In a VDI, {\em virtual desktops} are hosted in
VMs that reside in a data center. 
Users access virtual desktops via a remote display protocol. 
VDIs provides simplicity 
in administration and management: applications on virtual
desktops can be centrally added, deleted, upgraded and patched. 
After using server virtualization successfully, organizations 
are increasingly interested in investing in VDIs.
...
This is possible because of the very low steady-state CPU
overhead of a single virtual desktop and the aggressive
use of memory overcommit.

While the {\em steady-state} behavior of desktop VMs
allows many of them to be hosted on a single host,
...
CPU cycles, the host cannnot use transparent page sharing
effectively to retain a high VM density per host either. Bootstorms can be
...
can mitigate the stress on the storage subsystem in
...
and also allows data centers to sustain a high VM density per host. 


Data deduplication is partly effective because identical VMs load
